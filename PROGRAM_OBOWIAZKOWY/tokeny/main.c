...
{
	// TEST 1 DEKLARACJE
	char *pcTestString1 = " Deliemeter before first token";
	char *pcTestString2 = "     ";
	char *pcTestString3 = "More than   1 delimeter between";
	unsigned char ucHowManyTokens;
	
	// TEST 2 DEKLARACJE
	char pcKeywordReset[] = "reset"; 
	char pcKeywordLoad[] = "load";
	char pcKeywordStore[] = "store";
	enum KeywordCode eTestCode;
	
	// TEST 3 DEKLARACJE
	char pcMessageLoad[] = {"load 0x20 immediately"};
	char pcMessageStore[] = {"0x0A   later store "};
	char pcMessageReset[] = {"  0x0B reset now"};
	
	// TEST 1 
	ucHowManyTokens = ucFindTokensInString(pcTestString1);
	ucHowManyTokens = ucFindTokensInString(pcTestString2);
	ucHowManyTokens = ucFindTokensInString(pcTestString3);
	
	// TEST 2 
	eStringToKeyword(pcKeywordReset, &eTestCode);
	eStringToKeyword(pcKeywordLoad, &eTestCode);
	eStringToKeyword(pcKeywordStore, &eTestCode);
	
	// TEST 3 
	DecodeMsg(pcMessageLoad);
	DecodeMsg(pcMessageStore);
	DecodeMsg(pcMessageReset);
